...
use crate::claude_paths::{
    claude_file_exists, get_claude_dir,
    get_claude_metadata, list_claude_directory, read_claude_file, write_claude_file,
};
use crate::commands::projects_cache::{get_cached_projects, get_cached_project_sessions, invalidate_project_cache};
...
/// Lists all projects in the ~/.claude/projects directory (optimized with caching)
#[tauri::command]
pub async fn list_projects() -> Result<Vec<Project>, String> {
    log::info!("Listing projects from ~/.claude/projects (with cache)");

    // Use the cached version for fast loading
    let cached_projects = get_cached_projects(false).await?;
    
    // Convert cached projects to the expected format
    let mut projects = Vec::new();
    for cached_project in cached_projects {
        // Get sessions from cache
        let sessions = get_cached_project_sessions(cached_project.id.clone()).await?;
        
        projects.push(Project {
            id: cached_project.id,
            path: cached_project.path,
            sessions,
            created_at: cached_project.created_at,
        });
    }

    log::info!("Found {} projects (from cache)", projects.len());
    Ok(projects)
}

/// Force refresh the projects cache
#[tauri::command]
pub async fn refresh_projects_cache() -> Result<String, String> {
    log::info!("Force refreshing projects cache");
    get_cached_projects(true).await?;
    Ok("Projects cache refreshed".to_string())
}

/// Helper to get project path from sessions by project ID
fn get_project_path_from_sessions_by_id(project_id: &str) -> Result<String, String> {
    // Get list of sessions in this project
    let session_files = list_claude_directory(&format!("projects/{}", project_id))?;
    
    for session_file in session_files {
        if session_file.ends_with(".jsonl") {
            let session_path = format!("projects/{}/{}", project_id, session_file);
            if let Ok(content) = read_claude_file(&session_path) {
                // Read the first few lines to find the project_path
    ...
    Err("No project path found in sessions".to_string())
}

/// Gets sessions for a specific project (optimized with parallel processing)
#[tauri::command]
pub async fn get_project_sessions(project_id: String) -> Result<Vec<Session>, String> {
    ...
    let sessions: Vec<Session> = session_files
        .par_iter()
        .filter(|name| name.ends_with(".jsonl"))
        .filter_map(|session_file| {
            if let Some(session_id) = session_file.strip_suffix(".jsonl") {
                let session_path = format!("{}/{}", project_dir_path, session_file);
                
                // Get file creation time
                let created_at = get_claude_metadata(&session_path).unwrap_or(0);

                // Extract first user message and timestamp (only if needed)
                let (first_message, message_timestamp) = if should_load_session_details() {
                    extract_first_user_message_from_path(&session_path)
                } else {
                    (None, None)
                };

                // Try to load associated todo data (only if needed)
                let todo_path = format!("todos/{}.json", session_id);
                let todo_data = if should_load_todo_data() && claude_file_exists(&todo_path) {
                    read_claude_file(&todo_path)
                        .ok()
...
// Helper functions for lazy loading configuration
fn should_load_session_details() -> bool {
    // Can be made configurable - for now, always load
    true
}

fn should_load_todo_data() -> bool {
    // Can be made configurable - for now, always load
    true
}

/// Extract first user message from a session file using relative path
fn extract_first_user_message_from_path(relative_path: &str) -> (Option<String>, Option<String>) {
    if let Ok(content) = read_claude_file(relative_path) {
        for line in content.lines() {
            if let Ok(entry) = serde_json::from_str::<JsonlEntry>(line) {
                if let Some(message) = entry.message {
                    if message.role.as_deref() == Some("user") {
                        return (message.content, entry.timestamp);
                    }
                }
            }
        }
                    ...
                    if let Some(claude_session_id) = msg["session_id"].as_str() {
                        ...
                        if session_id_guard.is_none() {
                            *session_id_guard = Some(claude_session_id.to_string());
                            ...
                            // Invalidate project cache when new session is created
                            if let Some(project_id) = project_path_clone.split('/').last() {
                                let encoded_project_id = project_id.replace('/', "-");
                                invalidate_project_cache(Some(&encoded_project_id));
                            }
...
pub async fn restore_checkpoint(
    ...
    let session_path = format!("projects/{}/{}.jsonl", result.checkpoint.project_id, session_id);
...
pub async fn get_checkpoint_diff(
    ...
    let storage = CheckpointStorage::new(get_claude_dir().map_err(|e| e.to_string())?);
    // Note: CheckpointStorage still needs direct path access, will need to be updated separately
...
#[tauri::command]
pub async fn get_projects_cache_stats() -> Result<String, String> {
    Ok(crate::commands::projects_cache::get_cache_stats())
}

/// Clear the projects cache
#[tauri::command]
pub async fn clear_projects_cache() -> Result<String, String> {
    invalidate_project_cache(None);
    Ok("Projects cache cleared".to_string())
}
